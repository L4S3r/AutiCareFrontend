...
export default function DoctorDashboard({ language, doctorUser, onLogout }: DoctorDashboardProps) {
  ...
  const [activeTab, setActiveTab] = useState<'dashboard' | 'patients' | 'genetic' | 'chat' | 'settings'>('dashboard');
              ...
              {showUnassigned && (
                <div className="bg-gradient-to-br from-sky-50 to-indigo-50/50 border border-sky-200 rounded-3xl p-5 shadow-sm animate-fade-in">
                  <div className="flex items-center gap-2 mb-4">
                    ...
                    <h3 className="text-xs font-black text-sky-800 uppercase tracking-widest">{isRtl ? 'حالات بدون طبيب' : 'Unassigned Children'}</h3>
                  </div>
                  {loadingUnassigned ? (
                    ...
                    <p className="text-xs text-slate-400 italic font-medium">{isRtl ? 'جميع الحالات لديها طبيب معين.' : 'All children have an assigned doctor.'}</p>
                  ) : (
                      ...
                      {unassignedList.map((child: any) => (
                        <div key={child._id || child.id} className="flex items-center justify-between bg-white rounded-xl p-3 border border-sky-100 shadow-xs">
                          <div>
                            ...
                            {child.parentId && <p className="text-[9px] text-slate-400">{isRtl ? 'الوالد:' : 'Parent:'} {(child.parentId as any)?.name || child.parentId}</p>}
                          </div>
                          <button
                            onClick={async () => {
                              ...
                              try {
                                ...
                                if (res.success) {
                                  setActionSuccess(isRtl ? 'تم تعيين الحالة لك!' : 'Case assigned to you!');
                                  setUnassignedList(prev => prev.filter(c => (c._id || c.id) !== (child._id || child.id)));
                                  setTimeout(() => setActionSuccess(''), 4000);
                                }
                              } catch (err: any) {
                                setClinicalError(err?.message || (isRtl ? 'فشل التعيين' : 'Assignment failed'));
                                setTimeout(() => setClinicalError(''), 4000);
